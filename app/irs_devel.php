...
switch ($method) {
    ...
    case "bayar":
        ...
        break;
    case "cekbooking":
        echo cekbooking($raw_msg);
        break;
    case "bayarbooking":
        echo bayarbooking($raw_msg);
        break; 
    case "transaksi":
        echo transaksi($raw_msg);
        break;
    default :
...
function randomHp()
{
    $items = array("081216221775","081216318350","081219496485","081219550857","081222483287","081222691989","081232218510","081236736800","081242947201","081243240735","081244186501","081245760990","081248142424","081250404690","081251100243","081254984974","081258545824","081260217971","081266816078","081267719305","081268844251","081277770326","081278381431","081282388488","081283333463","08128475364","081285015843","081285015845","081285146462","081285539295","081285550385","081286001337","081286616893","08128888152 081289730000","081296520528","081297145560","081299179692","081355255781","081355403664","081356650199","081377771761","081383884531","081536732305","081574465202","081807115347","081908332685","081927614742","081953100931","081992419759","081998334687","081998405357","082116715460","082150649621","082166390334","082187359184","082187664020","082189938236","082190630447","082190634430","082190672986","082190726854","082191323309","082193947112","082225555781","082225932929","082247657898","082253041634","082255536359","082259710745","082271015660","082271415677","082290498271","082291616353","082292245411","082292245433","082296030948","082298090724","082311173627","082311284843","082311813117","082311884417","082312446655","08231844191","082322225678","082326141616","082330602071","082331354006","082331402694","082331432694","082335528313","0823357491130","082337830730","082338235158","082346654084");
   return $items[array_rand($items)];
}
function transaksi($req)
{
    $i = -1;
    $kdproduk   = trim(strtoupper($req['produk']));
    $idoutlet   = trim(strtoupper($req['uid']));
    $pin        = trim(strtoupper($req['pin']));
    $idpel1     = trim(strtoupper($req['tujuan']));
    $ref1       = trim(strtoupper($req['ref1']));
    $add        = trim(strtoupper($req['add']));
    $idpel2     = "";
    ...
    if(substr(strtoupper($kdproduk), 0,3) == "CEK"){ // CEK TRANSAKSI
        ...
        $params     = setMandatoryResponIrs($frm, $ref1, "", "","CEK", $req);
        ...
        $man        = FormatMsg::mandatoryPayment();
        $frm        = new FormatMandatory($man["inq"], $resp);    
        $params     = setMandatoryResponIrs($frm, $ref1, "", "","BAYAR", $req);
        $frm        = getParseProduk($kdproduk, $resp);
        if(substr(strtoupper($kdproduk), 0,5) == "BLTRF"){
            $adddata    = tambahdataproduk($kdproduk, $frm,$kodebank);
        }else{
        ...
        return joinimplode2($merge);
    }elseif(substr(strtoupper($kdproduk), 0,2) == "CS"){ // CEK STATUS
         $kdproduk   = substr($kdproduk,2); // GET ID PRODUK

    }elseif(substr(strtoupper($kdproduk), 0,3) == "PLS"){ // PULSA
         $kdproduk   = substr($kdproduk,3); // GET ID PRODUK
    }elseif(substr(strtoupper($kdproduk), 0,2) == "GM"){ // GAME
         $kdproduk   = substr($kdproduk,2); // GET ID PRODUK
    }elseif(substr(strtoupper($kdproduk), 0,3) == "EMY"){ // Emoney
         $kdproduk   = substr($kdproduk,3); // GET ID PRODUK
    }else{
...
function cekbooking($req){
    $i = -1;
    $kdproduk   = trim(strtoupper($req['produk']));
    $idoutlet   = trim(strtoupper($req['uid']));
    $pin        = trim(strtoupper($req['pin']));
    $kodepayment= trim(strtoupper($req['kodepayment']));
    ...
    // GET TOKEN & INSERT TOKEN
    $cek_token = cek_token($idoutlet);
    if(!empty($cek_token)){
        $get_token = $cek_token;
    }else{
         $result_sign =  json_decode(send_json($request_sign, "https://api.fastravel.co.id/app/sign_in"), true);
        if($result_sign["rc"] == "00"){
            $get_token = $result_sign["token"];
            insert_token($idoutlet, $get_token);
        }else{
              $msg_detail = array("status" => (string) trim($result_sign["rc"]),
                "keterangan" => (string) trim($result_sign["rd"]));
              $msg = joinimplode($msg_detail);
              echo $msg;
              die();
        }
    }
    // INQUIRY BOOKING
    $request_inquiry= array(
        "productCode" => $kdproduk,
        "paymentCode"  => $kodepayment,
        "token" => $get_token
    );

    $result_inquiry = json_decode(send_json($request_inquiry, "https://api.fastravel.co.id/train/inquiry_online_book"), true);

    if($result_inquiry["rc"] == "00"){
        $data_kodebooking = get_global_tiket("bill_info2", $result_inquiry["mid"]);
        $data_pemesanan = $result_inquiry["data"]["fieldStruk"];
        $data_add = explode("/", $data_pemesanan['kursi']);
        $data_pemesanan_detail = array(
                    "jenis"   => "CEK",
                    "tanggal" => $data_pemesanan['tanggal'], 
                    "kodebooking" => $data_kodebooking,
                    "noIdentitas" => $data_pemesanan['noIdentitas'], 
                    "nama" => $data_pemesanan['nama'], 
                    "namaKA" => $data_pemesanan['namaKA'], 
                    "tglKeberangkatan" => $data_pemesanan['tglKeberangkatan'], 
                    "tglKedatangan" => $data_pemesanan['tglKedatangan'], 
                    "stasiunAsal" => $data_pemesanan['stasiunAsal'], 
                    "stasiunTujuan" => $data_pemesanan['stasiunTujuan'], 
                    "kelas" => $data_add[1], 
                    "kursi" => $data_add[0], 
                    "nominal" => $data_pemesanan['nominal'], 
                    "biayaAdmin" => $data_pemesanan['biayaAdmin'], 
                    "diskonChannel" => $data_pemesanan['diskonChannel'], 
                    "totalBiaya" => $data_pemesanan['totalBiaya'] 
                );
        $msg =  joinimplode2($data_pemesanan_detail, $separator="/");
    }else{
        $data_error = array(
            "status" => $result_inquiry["rc"],
            "keterangan" => $result_inquiry["rd"]
        );
        $msg =  joinimplode2($data_error, $separator="/");
    }
    return $msg;
}

function bayarbooking($req){
    $i = -1;
    $kdproduk   = trim(strtoupper($req['produk']));
    $idoutlet   = trim(strtoupper($req['uid']));
    $pin        = trim(strtoupper($req['pin']));
    ...
    $get_token  = cek_token($idoutlet);
    $get_data_add = get_global_tiket_kai($idoutlet, $kdproduk, $kodepayment);

    $request_payment= array(
        "productCode" => $kdproduk,
        "paymentCode"  => $kodepayment,
        "transactionId" => $get_data_add['id_transaksi'],
        "normalSales" =>$get_data_add['nominal'],
        "extraFee" => 0,
        "discount" => 0,
        "token" => $get_token
    );

    $result_payment = json_decode(send_json($request_payment, "https://api.fastravel.co.id/train/payment_online_book"), true);


    if($result_inquiry["rc"] == "00"){
        $data_kodebooking = get_global_tiket("bill_info2", $result_payment["mid"]);
        $id_transaksi     = get_global_tiket("id_transaksi", $result_payment["mid"]);
        $id_outlet        = get_global_tiket("id_outlet", $result_payment["mid"]);
        $struk = "http://api.fastravel.co.id/app/generate_etiket?produk=kereta&transaction_id=".$id_transaksi."&id_outlet=".$id_outlet."";
        $data_pembayaran = array(
                    "jenis" => "BAYAR",
                    "status" => $result_payment["rc"],
                    "keterangan" => $result_payment["rd"],
                    "kodebooking" => $data_kodebooking,
                    "struk" => $struk
                );
        $msg =  joinimplode2($data_pembayaran, $separator="/");
    }else{
        $data_error = array(
            "status" => $result_payment["rc"],
            "keterangan" => $result_payment["rd"]
...
//         1 BLN
       ...
       if($idpel1=="06620177"){
        ...
        if($idpel1 == "162406307717"){
            ...
            $resp ="TAGIHAN*TVINDVS*121613693331387*10*20210219070852*H2H*510368942***000000055000*0*".$idoutlet."*------**734693661*1**30100*2048836225*00*MNC VISION/INDOVISION MENYATAKAN STRUK INI SEBAGAI BUKTI PEMBAYARAN YANG SAH, MOHON DISIMPAN TERIMA KASIH*0000000**510368942*01***SALIJAN SALIJAN ***000000055000*****116949SD*MNC****11/03/2021-10/04/2021***MNC Vision/INDOVISION MENYATAKAN STRUK INI SEBAGAI BUKTI PEMBAYARAN YANG SAH, MOHON DISIMPAN +Terima kasih *    ";
        }
...
function pulsa($req){
    ...
    if($kdproduk == 'I100H'){
    ...
    }else if($kdproduk == 'I20P'){
        if($nohp == "08112335757"){
            $resp = "PULSA*S100H*2877175367*9*20180911210836*H2H*08112335757*96750*".$idoutlet."*".$pin."**SK100**1548108783*1114460419*999*EXT: FAILED ";
        } else {
            $resp ="PULSA*TD2H*4783458425*4*20210727224627*H2H*0895630039496*20850*SP118170****------**2211690562*00*Pengisian pulsa TD2H Anda ke nomor 0895630039496 BERHASIL. SN=9314235045. Harga=20850   ";
        }
        ...
        $resp = "PULSA*S10H*808257112*10*".$tanggal."*H2H*".$nohp."*10850*".$idoutlet."*".$pin."**3023051**75625109*437881625*14*EXT: NO IDENTITAS PELANGGAN TIDAK DITEMUKAN";   
            ...
            $resp="PULSA*".$kdproduk."*805817646*10*".$tanggal."*H2H*".$nohp."*10000*".$idoutlet."*------*------***665254*437110932*00*SEDANG DIPROSES";
       }else{
...
function game($req) {
    ...
    if(strpos(strtoupper($kdproduk), 'ML') !== false){
        $resp = "GAME*TOLBR50H*4783211946*4*20210727185746*H2H*6013501803699643*50900*SP178229****6013501803699643/BRIZZI/50000/425679444367.**2211521112*00*Pembelian Voucher TOLBR50H BERHASIL. SN=6013501803699643/BRIZZI/50000/425679444367.    "     ;
        ...
        $resp = "GAME*GJD50H*4779952035*4*20210725060438*H2H*085931313178*50400*".$idoutlet."*------***R21627167874518/MICHAEL JIM IRWAN/085931313178/50000**2209125525*00*Pembelian Voucher GJD50H BERHASIL. SN=R21627167874518/MICHAEL JIM IRWAN/085931313178/50000 ";
    } else if(strpos(strtoupper($kdproduk), 'GJK') !== false){
        $resp = "GAME*GJK150H*121627166258406*16*20210725053745*H2H*081617333555*150800*HH13483*------**GOPAY-150000*GOPAY GP-6281617333555-002209119275*542511260*2209119275*00*Pembelian Voucher GJK150H BERHASIL. SN=GOPAY GP-6281617333555-002209119275. Harga=150800. Saldo=542511260" ;
...
// error_reporting(E_ALL);
    ...
    $man        = FormatMsg::mandatoryPayment();
    $frm        = new FormatMandatory($man["inq"], $resp);    
    $params     = setMandatoryResponNewDev($frm, $ref1, "", "", $req);
    $frm        = getParseProduk($kdproduk, $resp);
    if(substr(strtoupper($kdproduk), 0,5) == "BLTRF"){
        $adddata    = tambahdataproduk($kdproduk, $frm,$kodebank);
    }else{
...
function bayar($req){
    ...
    // get data idtrx, nominal, periode
    // $getdataCustom = getdataCustom($idoutlet, $kdproduk, $idpel1, $idpel2);
    // $id_transaksi = $getdataCustom[0];
    // if(substr($kdproduk, 0, 6) != 'PLNPRA'){
    //     $nominal = $getdataCustom[1]; 
    ...
    if ($frm->getStatus() == "00") {
        $url        = enkripUrl(strtoupper($idoutlet), $frm->getIdTrx());
        $url_struk  = array("url_struk" => "https://202.43.173.211/struk/?id=" . $url);
        $merge      = array_merge($params,$adddata,$adddata2,$url_struk);
    } else {
        $merge      = array_merge($params,$adddata,$adddata2);
    }
