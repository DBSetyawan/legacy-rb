...
class Message 
{
	...
	public function pack()
	{
		...
		if ($this->options['lengthPrefix'] > 0) {
			...
			$message = sprintf('%0' . $this->options['lengthPrefix'] . 'd', strlen($message) / 2) . $message;
		}
	...
	public function unpack($message)
	{
		...
		// 		throw new \Exception('Message length is ' . strlen($message) / 2 . ' and should be ' . $length);
		// 	}
		// }
	...
	public function setMTI($mti)
	{
		if (!preg_match('/^[0-9]{4}$/', $mti)) {
			throw new \Exception('Bad MTI field it should be 4 digits string');
		}
